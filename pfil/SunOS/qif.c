...
static	void		qif_callback(void *info);

static	int		qif_num = 0;
static	kmem_cache_t	*qif_cache = NULL;

kcondvar_t		qif_timewait;
kmutex_t		qif_timemtx;
#if SOLARIS2 >= 7
timeout_id_t		qif_timeout = NULL;
#else
int			qif_timeout = 0;
#endif
qif_t			*qif_head = NULL;
...
int qif_startup()
{
	...
	cv_init(&qif_timewait, "qif_timeout_cv", CV_DRIVER, NULL);
	mutex_init(&qif_timemtx, "qif_timeout_mtx", MUTEX_DRIVER, NULL);

	qif_timeout = timeout(qif_callback, NULL, 1000);
...
void qif_stop()
{
	...
	mutex_destroy(&qif_timemtx);
	cv_destroy(&qif_timewait);
}
	...
	queue_t *rq;
{
	...
	if (qif->qf_hl == 0) {
		...
		if (qif->qf_hl == 0 && ill->ill_type != IFT_OTHER) {
			cmn_err(CE_WARN,
				"!%s: Unknown layer 2 size. type %d sap %x\n",
...
/* ------------------------------------------------------------------------ */
/* Function:    qif_grabqif                                                 */
/* Returns:     qif_t * - NULL == search failed, else pointer to qif_t      */
/* Parameters:  name(I) - pointer to the name                               */
/*              sap(I)  - SAP value                                         */
/*                                                                          */
/* This function searches for a matching qif and returns with a lock on it  */
/* so that it can be safely used without danger of it being freed in a      */
/* manner that might result in these functions being called again.          */
/* ------------------------------------------------------------------------ */
qif_t *qif_grabqif(char *name, int sap)
{
	qif_t *qif;
...
qifpkt_t *pkthead = NULL;

/* ------------------------------------------------------------------------ */
/* For queuing up packets to come back up into IP */
/* Not for sending packets directly out of the system */
/* ------------------------------------------------------------------------ */
int qif_queuepkt(mblk_t *pkt, int inout, char *ifname, int sap)
{
	qif_t *qif;

	qif = qif_grabqif(ifname, sap);
	if (qif == NULL) {
		freemsg(pkt);
		/* increase counter */
		return -1;
	...
	if (inout == 0) {
		putnext(backq(RD(qif->qf_q)), pkt);
	} else {
		putnext(backq(WR(qif->qf_q)), pkt);
	}
...
/* ------------------------------------------------------------------------ */
/* Function:    qif_callback                                                */
/* Returns:     void                                                        */
/* Parameters:  info(I) - callback info pointer                             */
/*                                                                          */
/* This is a long lasting callback that acts to take care of tasks that get */
/* placed on its queue.  It understands that if the current job pointer     */
/* should ever be -1 then it is time to exit.  To avoid being a busy-wait,  */
/* it uses a kcondvar to sleep on and await notification of changes by those*/
/* that put things on its queue.                                            */
/* ------------------------------------------------------------------------ */
static void qif_callback(void *info)
{
	qifpkt_t *p;

	PRINT(5, (CE_CONT, "qif_callback(%p)\n", info));

	mutex_enter(&qif_timemtx);
	do {
		cv_wait(&qif_timewait, &qif_timemtx);

		PRINT(5, (CE_CONT, "qif_callback:pkthead %p\n", pkthead));

		if (pkthead == (qifpkt_t *)-1) {
		...
		pkthead = p->qp_next;
		mutex_exit(&qif_timemtx);
		qif_queuepkt(p->qp_mb, p->qp_inout, p->qp_ifname, p->qp_sap);
...
/* ------------------------------------------------------------------------ */
/* Function:    qif_addinject                                               */
/* Returns:     void                                                        */
/* Parameters:  qp(I) - pointer packet information                          */
/*                                                                          */
/* This function takes a collection of packet information as found in the   */
/* structure pointed to by qp and schedules it for delivery at a later date */
/* via a never-ending callout.  Since the callout sleeps, we need to wake   */
/* it up to activate the delivery.  The caller is not expecting the data to */
/* be returned so if we can't use it, we need to free it all up.            */
/* ------------------------------------------------------------------------ */
void qif_addinject(qifpkt_t *qp)
{
	qifpkt_t *p;

	PRINT(8, (CE_CONT, "qif_addinject(%p)\n", qp));
	if ((qp != NULL) && (qp != (qifpkt_t *)-1))
		qp->qp_next = NULL;
...
dropqifpkt:
	...
	if (pkthead == NULL) {
		...
		for (p = pkthead; (p != NULL) && p->qp_next; )
			p = p->qp_next;
		if (p == (qifpkt_t *)-1)
			goto dropqifpkt;
		p->qp_next = qp;
...
/* ------------------------------------------------------------------------ */
/* Function:    qif_fromill                                                 */
/* Returns:     qif_t *                                                     */
/* Parameters:  ill(I) - pointer to an ill_t                                */
/*                                                                          */
/* Given an ill_t pointer, find and return the matching qif_t pointer       */
/* ------------------------------------------------------------------------ */
qif_t *qif_fromill(ill_t *ill)
{
	qif_t *qf;
...
/* ------------------------------------------------------------------------ */
/* Function:    qif_illrouteto                                              */
/* Returns:     qif_t *                                                     */
/* Parameters:  v(I)    - IP protocol version for the address               */
/*              addr(I) - pointer to address                                */
/*                                                                          */
/* Return a pointer to a an interface (represented by a qif_t *) that we    */
/* would normally use to send a packet desinted for the address stored at   */
/* "addr".                                                                  */
/* ------------------------------------------------------------------------ */
...
# endif
				       ...
				       MATCH_IRE_RECURSIVE);
#else
		dir = ire_lookup(ip.s_addr);
#endif
	}
#ifdef USE_INET6
	else if (v == 6) {
		dir = ire_route_lookup_v6(addr, NULL, 0, 0, NULL, NULL, NULL,
					  MATCH_IRE_DSTONLY|MATCH_IRE_DEFAULT|
					  MATCH_IRE_RECURSIVE);
	}
#endif
