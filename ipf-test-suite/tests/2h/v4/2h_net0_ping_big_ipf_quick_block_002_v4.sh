...
gen_ipf_conf() {
	generate_test_hdr
	cat <<__EOF__
block in quick on ${SUT_NET0_IFP_NAME} proto icmp from any to ${SUT_NET0_ADDR_V4}
block out quick on ${SUT_NET0_IFP_NAME} proto icmp from ${SUT_NET0_ADDR_V4} to any
__EOF__
	generate_pass_rules
	return 0;
}
