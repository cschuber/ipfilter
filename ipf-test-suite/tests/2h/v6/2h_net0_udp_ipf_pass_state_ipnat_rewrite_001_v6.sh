...
gen_ipf_conf() {
	...
	generate_test_hdr
	cat <<__EOF__
pass out on ${SUT_NET0_IFP_NAME} proto udp from any to any keep state
__EOF__
	return 0;
}
...
do_test() {
	start_udp_server ${SENDER_CTL_HOSTNAME} ${SENDER_NET0_ADDR_V6_A1} 5051
	sleep 1
	udp_test ${SUT_CTL_HOSTNAME} ${SENDER_NET0_ADDR_V6} 5051 pass
	ret=$?
