...
pass in on ${SUT_NET0_IFP_NAME} proto icmp from any to any with frag-body
pass out on ${SUT_NET0_IFP_NAME} proto icmp from any to any icmp-type echorep
pass out on ${SUT_NET0_IFP_NAME} proto icmp from any to any with frag-body
pass out on ${SUT_NET1_IFP_NAME} proto icmp from any to any icmp-type echo
pass out on ${SUT_NET1_IFP_NAME} proto icmp from any to any with frag-body
pass in on ${SUT_NET1_IFP_NAME} proto icmp from any to any icmp-type echorep
pass in on ${SUT_NET1_IFP_NAME} proto icmp from any to any with frag-body
__EOF__
	return 0;
}
