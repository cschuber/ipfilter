...
gen_ipf_conf() {
	...
	generate_test_hdr
	cat << __EOF__
pass in on ${SUT_NET0_IFP_NAME} proto icmp all
pass out on ${SUT_NET0_IFP_NAME} proto icmp all
pass in on ${SUT_NET1_IFP_NAME} proto icmp all
pass out on ${SUT_NET1_IFP_NAME} proto icmp all
__EOF__
	return 0;
}
