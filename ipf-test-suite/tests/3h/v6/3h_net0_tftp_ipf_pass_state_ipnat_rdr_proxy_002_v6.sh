...
gen_ipnat_conf() {
	cat <<__EOF__
rdr ${SUT_NET0_IFP_NAME} ${RECEIVER_NET1_ADDR_V6_A1} -> ${RECEIVER_NET1_ADDR_V6} proxy port tftp tftp/udp
__EOF__
	return 0;
}
...
do_test() {
	tftp_test ${SENDER_CTL_HOSTNAME} ${RECEIVER_NET1_ADDR_V6_A1} ${FTP_PATH} pass
	return $?;
}
