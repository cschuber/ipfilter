...
logging_start() {
	...
	if [[ $1 = 2h || $1 = 3h ]] ; then
		${RRSH} ${SENDER_CTL_HOSTNAME} ${IPF_BIN_DIR}/capture.sh \
		    ${SENDER_NET0_IFP_NAME} ${LOGS_FILE} ${PIDS_FILE} &
	fi
	if [[ $1 = 3h ]] ; then
		${RRSH} ${RECEIVER_CTL_HOSTNAME} ${IPF_BIN_DIR}/capture.sh \
...
logging_stop() {
	...
	if [[ $1 = 2h || $1 = 3h ]] ; then
		${RRSH} ${SENDER_CTL_HOSTNAME} ${IPF_BIN_DIR}/killpid.sh \
		    ${PIDS_FILE}
	fi
	if [[ $1 = 3h ]] ; then
		${RRSH} ${RECEIVER_CTL_HOSTNAME} ${IPF_BIN_DIR}/killpid.sh \
		    ${PIDR_FILE}
	fi
}
