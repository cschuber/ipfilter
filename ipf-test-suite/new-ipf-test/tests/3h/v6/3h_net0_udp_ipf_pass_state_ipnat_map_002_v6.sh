...
gen_ipnat_conf() {
	cat <<__EOF__
map ${SUT_NET1_IFP_NAME} ${SUT_NET1_ADDR_V6} -> ${NET1_FAKE_ADDR_V6} portmap udp 10000:11000
__EOF__
	return 0;
}
...
do_test() {
	...
	sleep 3
	udp_test ${SENDER_CTL_HOSTNAME} ${RECEIVER_NET1_ADDR_V6} 5050 pass
	ret=$?
...
do_verify() {
	${IPF_BIN_DIR}/log.sh verify_srcdst_1 \
	    ${NET1_FAKE_ADDR_V6} ${RECEIVER_NET1_ADDR_V6}
	if [[ $? -eq 0 ]] ; then
